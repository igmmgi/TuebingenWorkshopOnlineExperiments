...
jsPsych.plugins["pavlovia"] = (function () {
  ...
  const defaultErrorCallback = function (error) {
    ...
    let htmlCode =
      "<h3>[jspsych-pavlovia plugin " + version + "] Error</h3><ul>";
    while (true) {
   ...
   *
   * @param {Object} trial - the jsPsych trial
   * @param {string} [configURL= "config.son"] - the URL of the pavlovia.org json configuration file
  ...
  const _configure = async function (configURL) {
    ...
    try {
      ...
      urlParameters.forEach((value, key) => {
        if (key.indexOf("__") === 0) _serverMsg.set(key, value);
      });
  ...
  const _openSession = function () {
    let response = {
      origin: "_openSession",
      context:
        "when opening a session for experiment: " + _config.experiment.fullpath,
    };
    ...
    return new Promise((resolve, reject) => {
      const url =
        _config.pavlovia.URL +
        "/api/v2/experiments/" +
        encodeURIComponent(_config.experiment.fullpath) +
        "/sessions";
      $.post(url, data, null, "json")
        .done((data, textStatus) => {
          ...
          _config.experiment.saveFormat = Symbol.for(
            data.experiment.saveFormat
          );
          _config.experiment.saveIncompleteResults =
            data.experiment.saveIncompleteResults;
  ...
  const _save = async function (trial, data, sync = false) {
    ...
    if (
      _config.experiment.status === "RUNNING" &&
      !_serverMsg.has("__pilotToken")
    ) {
      ...
      return {
        origin: "_save",
        context:
          "when saving results for experiment: " + _config.experiment.fullpath,
